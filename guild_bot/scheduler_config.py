...
#         if not event.is_notification: # Не нотис, нужно досрочное напоминание
#             # --- Напоминание за offset минут ---
#             offset = 11
#             reminder_hours = h
#             reminder_minutes = m - offset
...
#             scheduler.add_job(
#                 notif_service.send_event_reminder,
#                 CronTrigger(day_of_week=reminder_day,
#                             hour=reminder_hours,
#                             minute=reminder_minutes,
#                             timezone=config.TIME_ZONE),
...
#                 args=[event]
#             )
#         # print(f"🔔 Добавлено напоминание для {event['name']} в {reminder_hours:02d}:{reminder_minutes:02d}")
#         else:
#             notification_count += 1

#         # --- Уведомление о начале события или создание нотиса ---
#         scheduler.add_job(
#             notif_service.send_event_start,
#             CronTrigger(day_of_week=event.day,
#                         hour=h,
#                         minute=m,
#                         timezone=config.TIME_ZONE),
#             id=f'{"notification" if event.is_notification else "start"}_{i}',
#             args=[event]
#         )
#     print(f"\n✅ Всего добавлено задач: {len(scheduler.get_jobs())}\n📊 Из них нотисов:{notification_count}\n")